...
if [ "$skipUpdate" = "no" ]; then
	...
	if [ "$?" != "0" -a -s /root/server-check-new.sh ]; then
		...
		echo -e "${CY}-->${CL} A different version of the script was downloaded."
		echo -e "${CY}-->${CL} The server-check.sh updated! ${YW}Please re-run the script.${CL}"
		### A.E. ### just ask user to re-run the script for now. 
...
### A.E. ### Do we need RPi support? armhf is old RPi, new RPi is AARCH64? Maybe everybody remains a "pc" for simplicity
...
if [ "$?" = "0" ]; then
	...
	if [ $? != "0" ]; then
		...
		problem-report "Use the adduser command to create an account for $firstname"
	else
